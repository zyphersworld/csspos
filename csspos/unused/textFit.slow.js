...
(function(root, factory) {
  ...
  function processItem(el, settings){
    ...
    if (!originalWidth || (!settings.widthOnly && !originalHeight)) {
      ...
      if (window.console && !settings.suppressErrors) {
        if(!settings.widthOnly)
          console.error('Set a static height and width on the target element ' + el.outerHTML +
            ' before using textFit!');
        else
          console.error('Set a static width on the target element ' + el.outerHTML +
            ' before using textFit!');
      }
      return false;
    }
    ...
    var fitInterval = setInterval(fit, 0);
